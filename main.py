...
db = SQLAlchemy(app)
app.secret_key = 'y335T78Zr&4'


class Task(db.Model):
#these are our columns that are made with db.create_all()
...
def login():
    if request.method == 'POST':
        ...
        if user and user.password == password:
            ...
            # flash message why login failed
...
def register():
    if request.method == 'POST':
        ...
        if not existing_user:
            ...
            # TODO use better response message
...
def index():
    ...
    if request.method == 'POST':
        ...
        # assign task to user by getting owner
        new_task = Task(task_name, owner)
        db.session.add(new_task)
        db.session.commit()  
    
    #query all objects in Task
    #set up so the filter only works on the owner logged in.
